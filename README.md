...
The vkcube-specific rasterizer uses 20x20 tiles at 4K and 32x32
tiles at 8K to reduce classification overhead without removing per-pixel
coverage or depth tests. The 4K and 8K paths add one swapchain image, within
the advertised four-image limit, to absorb rare producer stalls without
expanding the CPU set.
...
pinned CPU for raster and presentation work, but ZPU never schedules work on a
third CPU. Large color, depth, and XCB SHM caches are bound before first touch
to the selected node and receive transparent huge-page advice; ZPU never
expands the CPU mask supplied by the caller.
