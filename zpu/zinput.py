...
def _library_candidates():
    """Return candidate paths for libzinput.so, ordered from most to least specific."""
    this_dir = os.path.dirname(os.path.realpath(__file__))
    candidates = [os.path.join(this_dir, "libzinput.so")]
    # When running from a source checkout, the C library is usually under tools/.
    repo_root = os.path.dirname(this_dir)
    candidates.append(os.path.join(repo_root, "tools", "libzinput.so"))
    # Allow the loader to search LD_LIBRARY_PATH and system paths.
    candidates.append("libzinput.so")
    return candidates
...
def load_library(path: Optional[str] = None) -> ctypes.CDLL:
    ...
    for p in candidates:
        ...
        raise OSError(
            "libzinput.so not found; build it with `make -C tools libzinput.so` "
            "or `python -m build`"
        ) from last_error
