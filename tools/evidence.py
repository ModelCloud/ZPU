...
def metric_rows(two, three, raw2d, raw3d, commit, utc):
    rows=[]
...
def progress_text(two,three,raw2d,raw3d,commit,utc,evidence):
    rows=metric_rows(two,three,raw2d,raw3d,commit,utc)
...
def progress(args):
    ...
    expected=progress_text(two,three,args.two_d,args.three_d,args.source_commit,args.utc,evidence)
...
def main():
    ...
    q=sub.add_parser("progress"); q.add_argument("--2d",dest="two_d",required=True); q.add_argument("--3d",dest="three_d",required=True); q.add_argument("--output",required=True); q.add_argument("--source-commit",required=True); q.add_argument("--utc",required=True); q.add_argument("--evidence-commit"); q.add_argument("--write",action="store_true"); q.add_argument("--skip-git-binding",action="store_true")
