...
AVX-512 rows are emitted only when the compiled binary and host report that backend available. Hosts without AVX-512 intentionally have no AVX-512 execution to verify; this is reported as unavailable rather than treated as a passing measurement.
