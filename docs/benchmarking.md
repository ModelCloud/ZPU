...
`tools/limited-cpus.sh zig build benchmark -Doptimize=ReleaseFast -- --json` runs the versioned `zpu-2d-app-scenes-v5-240x240-seed-151521030` workload. It measures clear, pixel writes, deliberately clipped rectangles, straight-alpha source-over blends, clipped RGBA sprite draws, complete deterministic frames, and four app-shaped scenes: `desktop_windows` (window shadows, panels, title bars, and controls), `terminal_cells` (a 30×30 glyph grid from an atlas plus cursor/selection overlays), `game_scene` (a 30×20 tile map, 96 sprites, and particles), and `design_canvas` (translucent layers, selection handles, and guides). Vulkan host-memory fill/copy operations remain included. Constant-color spans, per-pixel RGBA sprite spans, atlas regions, and colored rectangle batches use the selected scalar or vector backend. Raster operations run with scalar, every CPU/OS-available vector backend, and normal runtime dispatch; Vulkan host-memory operations have no raster dispatch choice and run once. Results include pixels/s (serialized as `mpix_s`), bytes/s and modeled GiB/s, submitted primitive draws/s, frames/s (`fps`), per-operation p50/p95/p99 latency, iteration counts, and fixed-output FNV-1a checksums.
...
Effective bytes are an operation model, not measured hardware memory bandwidth. Writes count four bytes/pixel; alpha blending counts four bytes read plus four written. The clipped-rectangle case counts its exact 12,060 in-bounds pixels (48,240 bytes). The sprite rate reports complete draw calls while its byte model counts the submitted 128×8×8 pixels (65,536 bytes), including clipped fragments; a frame models its 230,400-byte clear plus 131,072 bytes for 64 blended 16×16 rectangles, or 361,472 bytes total. The four app scenes have hand-computed traffic models and primitive draw counts: desktop 622,848 bytes/43 draws, terminal 692,864 bytes/903 draws, game 776,704 bytes/825 draws, and design 662,528 bytes/353 draws. Their geometry, source atlas, alpha masks, draw order, clipping, and checksums are fixed by independent reference-renderer tests, so an empty loop or repeated single coordinate cannot satisfy the gate.

Atlas-backed draws inspect the immutable source alpha once per call. If every source alpha is binary, transparent pixels take a no-op path and opaque pixels take a direct format-aware copy path in the scalar and vector kernels; RGBA8 mixed groups select packed source or destination bytes without channel unpacking, while BGRA8 uses the channel-swapping variant. Arbitrary-alpha sources continue through exact straight-alpha source-over arithmetic. This optimization is intentionally data-dependent rather than baked into the oracle: the terminal atlas is binary, while the game sprites remain seeded arbitrary-alpha data.

Rectangle batches select their row kernel once per clipped draw instead of once per scanline. Packed SIMD spans load/store words directly through explicitly unaligned views, and narrow (at most two-pixel) fill spans use direct format-aware writes because they cannot fill a vector lane. The v9 AVX2 boundary now carries multi-row rectangle and sprite kernels, preserving one safety check and one ABI transition per draw instead of per scanline. In the controlled v9 probe this reduced runtime `clipped_rectangle` p50 from 5.05 µs to 3.41 µs (1.48×), `frame` from 14.76 µs to 13.56 µs, and `game_scene` from 69.2 µs to 53.9 µs (1.29×); the full scene oracle and all backend checksums remain authoritative.
...
`ZPU_MAX_THREADS=2 tools/limited-cpus.sh zig build benchmark-3d-apps -Doptimize=ReleaseFast -- --json` runs three separate 800×600 two-core profiles through the existing counted CPU 3D draw API: layered desktop windows, terminal glyph quads, and a dynamic game-engine mesh scene. Each profile includes a full color/depth clear and its complete draw-call sequence. The desktop profile has 36 draws/72 triangles, the terminal profile has 241 draws/482 triangles, and the game profile has 32 draws/384 triangles. The terminal changes atlas-selected glyphs each frame; the game changes transforms and depth each frame; static replay is never used.

These profiles intentionally measure renderer behavior shaped like application submissions, not native Windows compositor, terminal, or game-engine integrations. Their unit-test contract renders each profile through serial and parallel paths and requires matching color/depth output and checksums, stable triangle counts, nonzero coverage, and a changed-but-valid animated game frame. See [3d-app-benchmarks.md](3d-app-benchmarks.md) for the exact workload construction, report fields, scenario selection, and smoke mode. The canonical vkcube readiness workload remains separate.
