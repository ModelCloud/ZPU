...
its VkFormat, image-type, sample-count, usage, and tiling domains before
reporting the truthful zero-sparse result.  Core and extension-defined format
enum values are distinguished from unknown integers, so valid unsupported
formats still return zero while malformed and stale calls preserve the
caller's count; the valid and rejection paths remain allocation-free.
...
the profile has no shared-memory or atomic state. `OpMemoryBarrier` is
accepted under the matching workgroup-scope/relaxed-semantics form and is
lowered to the same no-op. Stronger barrier scopes or memory semantics remain
...
- [ ] **A7 — compute:** compute pipeline records, dispatch envelopes, and the
  ...
  workgroup control- and memory-barrier forms are covered by
  malformed/unsupported and warm-path frontend tests without changing the
