...
draw with contiguous draws coalesced into one bounded batch. The target for
this pass is a 2× p50 frame-time improvement.
...
| --- | --- | --- |
| `wezterm_terminal` | [WezTerm](https://github.com/wezterm/wezterm), whose WebGPU front end can select Vulkan | 120×40 terminal cells plus a background; 4,801 draws in one bounded batch |
...
terminal profile intentionally fits under the 8,192-command batch limit, keeping
the complete frame in one bounded submission. Driver tests continue to
