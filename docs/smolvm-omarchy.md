...
`smol-machines/smolvm` exactly 1.16.1. The exact pin makes the guest workflow
...
SmolVM 1.16.1 `machine update --name NAME --no-net` operation while it is
...
- SmolVM exactly 1.16.1 available as `smolvm`, with help output proving
