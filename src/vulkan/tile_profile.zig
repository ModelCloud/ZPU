...
pub fn choose(caps: Capabilities, compiled: CompiledKernels) Profile {
    ...
    const can_avx2 = (host == .avx2 or host == .avx512) and compiled.hasMosaicAvx2();
    const can_avx512 = host == .avx512 and compiled.hasMosaicAvx512();
...
test "kernel linkage gates executable ISA" {
    ...
    const avx2 = choose(host_avx512, .{ .mosaic_primitive_avx2 = true });
    ...
    const avx512 = choose(host_avx512, .{ .mosaic_primitive_avx2 = true, .mosaic_pixel_avx512 = true });
...
test "OS vector state gates host class" {
    const no_ymm = choose(.{ .avx = true, .osxsave = true, .avx2 = true }, .{ .mosaic_primitive_avx2 = true });
    try std.testing.expectEqual(IsaClass.portable_vector, no_ymm.executable_class);
    const no_zmm = choose(.{ .avx = true, .osxsave = true, .ymm_state = true, .avx2 = true, .avx512f = true }, .{ .mosaic_primitive_avx2 = true, .mosaic_pixel_avx512 = true });
...
test "profile geometry remains aligned" {
    ...
    const profiles = [_]Profile{ choose(.{}, .{}), choose(caps, .{ .mosaic_primitive_avx2 = true }), choose(caps, .{ .mosaic_primitive_avx2 = true, .mosaic_pixel_avx512 = true }) };
