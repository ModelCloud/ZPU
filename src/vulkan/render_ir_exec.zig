...
fn packHalf(value: f32) u16 {
    // GLSL's half pack operation preserves finite values and infinities in
    // the binary16 domain.  Canonicalize NaNs so the bounded executor has a
    // stable payload independent of the host floating-point implementation.
    if (std.math.isNan(value)) return 0x7e00;
    const half: f16 = @floatCast(value);
    return @bitCast(half);
}

fn unpackHalf(bits: u16) f32 {
    const half: f16 = @bitCast(bits);
    const value: f32 = @floatCast(half);
    if (std.math.isNan(value)) return @bitCast(@as(u32, 0x7fc00000));
    return value;
}
...
pub const Vp9ColorTransformPlan = struct {
    luma_coordinate_interface: u32,
    chroma_coordinate_interface: u32,
    uniform_interface: u32,
    luma_image_interface: u32,
    chroma_image_interface: u32,
    output_interface: u32,
    bias_literal: [4]u8,
};
...
fn colorTransformMemberMatches(member: ir.UniformMember, ty: ir.Type, offset: u32, count: u32, stride: u32) bool {
    return same(member.ty, ty) and member.offset == offset and member.array_count == count and member.array_stride == stride;
}

fn detectChromiumVp9ColorTransform(program: *const ir.Program) ?Vp9ColorTransformPlan {
    const f32_scalar = ir.Type{ .scalar = .f32 };
    const f32x2 = ir.Type{ .scalar = .f32, .columns = 2 };
    const f32x3 = ir.Type{ .scalar = .f32, .columns = 3 };
    const f32x4 = ir.Type{ .scalar = .f32, .columns = 4 };
    const f32x3x3 = ir.Type{ .scalar = .f32, .columns = 3, .rows = 3 };
    if (program.stage != .fragment or program.instructions.len != 440 or program.interfaces.len != 8 or
        !std.mem.eql(u8, &program.identity.digest, &chromium_vp9_color_transform_identity)) return null;
    const color = program.interfaces[0];
    const luma_coordinate = program.interfaces[1];
    const chroma_coordinate = program.interfaces[2];
    const facing = program.interfaces[3];
    const output = program.interfaces[4];
    const uniform = program.interfaces[5];
    const luma_image = program.interfaces[6];
    const chroma_image = program.interfaces[7];
    if (color.storage != .input or !same(color.ty, f32x4) or color.location == null or color.location.? != 0 or
        luma_coordinate.storage != .input or !same(luma_coordinate.ty, f32x2) or luma_coordinate.location == null or luma_coordinate.location.? != 1 or
        chroma_coordinate.storage != .input or !same(chroma_coordinate.ty, f32x2) or chroma_coordinate.location == null or chroma_coordinate.location.? != 2 or
        facing.storage != .input or !same(facing.ty, .{ .scalar = .bool }) or !facing.builtin_front_facing or
        output.storage != .output or !same(output.ty, f32x4) or output.location == null or output.location.? != 0 or
        uniform.storage != .uniform or !uniform.block or uniform.descriptor_set == null or uniform.descriptor_set.? != 0 or uniform.binding == null or uniform.binding.? != 0 or uniform.member_count != 8 or
        luma_image.storage != .sampled_image or !same(luma_image.ty, f32x4) or luma_image.descriptor_set == null or luma_image.descriptor_set.? != 1 or luma_image.binding == null or luma_image.binding.? != 0 or
        chroma_image.storage != .sampled_image or !same(chroma_image.ty, f32x4) or chroma_image.descriptor_set == null or chroma_image.descriptor_set.? != 1 or chroma_image.binding == null or chroma_image.binding.? != 1) return null;
    if (!colorTransformMemberMatches(uniform.members[0], f32x3x3, 16, 1, 0) or
        !colorTransformMemberMatches(uniform.members[1], f32x3x3, 64, 1, 0) or
        !colorTransformMemberMatches(uniform.members[2], f32x3x3, 112, 1, 0) or
        !colorTransformMemberMatches(uniform.members[3], f32x3, 160, 1, 0) or
        !colorTransformMemberMatches(uniform.members[4], f32x3x3, 176, 1, 0) or
        !colorTransformMemberMatches(uniform.members[5], f32_scalar, 224, 7, 16) or
        !colorTransformMemberMatches(uniform.members[6], f32x3x3, 336, 1, 0) or
        !colorTransformMemberMatches(uniform.members[7], f32_scalar, 384, 7, 16)) return null;
    return .{
        .luma_coordinate_interface = 1,
        .chroma_coordinate_interface = 2,
        .uniform_interface = 5,
        .luma_image_interface = 6,
        .chroma_image_interface = 7,
        .output_interface = 4,
...
pub const Executor = struct {
    ...
    /// Return the exact resolved ABI of the captured Chromium VP9 Y/UV
    /// color-transform fragment. Any altered canonical IR is interpreter-only.
    pub fn vp9ColorTransformPlan(self: *const Executor) ?Vp9ColorTransformPlan {
        return switch (self.fast_path orelse return null) {
            .vp9_color_transform => |plan| plan,
            else => null,
        };
    }
    ...
    fn colorTransformTransfer(value: f32, uniform: []const u8, base: usize) Error!f32 {
        ...
        const exponent = try uniformF32(uniform, base);
        const nonlinear_scale = try uniformF32(uniform, base + 16);
        const nonlinear_bias = try uniformF32(uniform, base + 32);
        const linear_scale = try uniformF32(uniform, base + 48);
        const threshold = try uniformF32(uniform, base + 64);
        const nonlinear_offset = try uniformF32(uniform, base + 80);
        const linear_offset = try uniformF32(uniform, base + 96);
    ...
    fn loadColorTransformMatrix(uniform: []const u8, base: usize) Error![3][3]f32 {
        var matrix: [3][3]f32 = undefined;
        for (0..3) |column| {
            for (0..3) |row| matrix[column][row] = try uniformF32(uniform, base + column * 16 + row * 4);
        }
        return matrix;
    }
    ...
    fn vectorTimesColorTransformMatrix(vector: [3]f32, matrix: [3][3]f32) [3]f32 {
        ...
        for (0..3) |column| {
            var sum: f32 = 0;
            for (0..3) |row| sum += vector[row] * matrix[column][row];
            result[column] = canonicalF32(sum);
        }
        return result;
    }

    fn colorTransformMatrixTimesVector(matrix: [3][3]f32, vector: [3]f32) [3]f32 {
        var result: [3]f32 = undefined;
        for (0..3) |row| {
            var sum: f32 = 0;
            for (0..3) |column| sum += matrix[column][row] * vector[column];
            result[row] = canonicalF32(sum);
        }
        return result;
    }

    fn executeVp9ColorTransformResolved(luma_coordinate_bytes: []const u8, chroma_coordinate_bytes: []const u8, uniform: []const u8, luma_image: SampledImage, chroma_image: SampledImage, bytes: []u8) Error!void {
        if (uniform.len < 484 or bytes.len < 16) return error.Bounds;
        ...
        const luma = try sample(luma_image, luma_coordinates, bias);
        const chroma = try sample(chroma_image, chroma_coordinates, bias);
        var source = [_]f32{ @bitCast(luma.bits[0]), @bitCast(chroma.bits[0]), @bitCast(chroma.bits[1]) };
        const source_matrix = try loadColorTransformMatrix(uniform, 112);
        source = vectorTimesColorTransformMatrix(source, source_matrix);
        for (0..3) |lane| source[lane] = colorTransformClamp(canonicalF32(source[lane] + try uniformF32(uniform, 160 + lane * 4)));
    ...
    fn executeFastPath(fast_path: FastPath, bindings: []const Binding, outputs: []const Output) Error!void {
        switch (fast_path) {
            ...
            .vp9_color_transform => |path| {
                const luma_coordinates = try findBindingRecord(bindings, path.luma_coordinate_interface);
                const chroma_coordinates = try findBindingRecord(bindings, path.chroma_coordinate_interface);
                const uniform = try findBindingRecord(bindings, path.uniform_interface);
                if (uniform.sampled_image != null or uniform.input_attachment != null) return error.InvalidStorage;
                var output: ?[]u8 = null;
                for (outputs) |candidate| if (candidate.interface == path.output_interface) {
            ...
            switch (instruction.op) {
                ...
                .f_ldexp => {
                    ...
                    for (0..result.lanes()) |i| {
                        ...
                        // GLSL.std.450 gives poison for non-finite inputs and
                        // single-precision exponents above +128. Keep the
                        // result failure-atomic by checking each lane before
                        // publishing it to the program value table.
                        if (!std.math.isFinite(x) or exp > 128) return error.NumericDomain;
                        const scaled = std.math.ldexp(x, exp);
                        if (!std.math.isFinite(scaled)) return error.NumericDomain;
                ...
                .fadd, .fsub, .fmul, .fdiv, .frem, .fmod, .f_min, .f_max, .f_n_min, .f_n_max, .f_step => {
                    ...
                    for (0..result.lanes()) |i| {
                        ...
                        const z = switch (instruction.op) {
                            ...
                            .f_n_min => if (std.math.isNan(x)) y else if (std.math.isNan(y)) x else if (y < x) y else x,
                            .f_n_max => if (std.math.isNan(x)) y else if (std.math.isNan(y)) x else if (x < y) y else x,
                ...
                .f_clamp, .f_n_clamp, .u_clamp, .i_clamp => {
                    ...
                    for (0..result.lanes()) |i| {
                        if (instruction.op == .f_clamp or instruction.op == .f_n_clamp) {
                            ...
                            const lower = if (instruction.op == .f_n_clamp and std.math.isNan(x)) min_value else if (instruction.op == .f_n_clamp and std.math.isNan(min_value)) x else if (x < min_value) min_value else x;
                            const clamped = if (instruction.op == .f_n_clamp and std.math.isNan(lower)) max_value else if (instruction.op == .f_n_clamp and std.math.isNan(max_value)) lower else if (max_value < lower) max_value else lower;
                            result.bits[i] = canonicalFloat(@bitCast(clamped));
...
fn validate(program: *const ir.Program) Error!void {
    ...
    for (program.instructions, 0..) |instruction, pc| {
        ...
        const arity_ok = switch (instruction.op) {
            ...
            .f_clamp, .u_clamp, .i_clamp, .f_n_clamp, .f_mix, .fma, .f_smooth_step, .f_face_forward, .f_refract => n == 3,
            .f_distance, .f_cross, .f_reflect, .f_ldexp => n == 2,
        ...
        for (instruction.operands, 0..) |operand, oi| if (isValueOperand(instruction.op, oi)) {
            ...
            switch (instruction.op) {
                ...
                .i_pack_half2x16 => if (source_ty.scalar != .f32 or source_ty.columns != 2 or source_ty.rows != 1 or instruction.ty.columns != 1 or instruction.ty.rows != 1 or (instruction.ty.scalar != .i32 and instruction.ty.scalar != .u32)) return error.InvalidType,
                .f_unpack_half2x16 => if ((source_ty.scalar != .i32 and source_ty.scalar != .u32) or source_ty.columns != 1 or source_ty.rows != 1 or instruction.ty.scalar != .f32 or instruction.ty.columns != 2 or instruction.ty.rows != 1) return error.InvalidType,
                ...
                .ieq, .ine, .ugt, .uge, .ult, .ule, .sgt, .sge, .slt, .sle => if (source_ty.scalar != .i32 and source_ty.scalar != .u32 or source_ty.columns < 1 or source_ty.columns > 4 or source_ty.rows != 1) return error.InvalidType,
        ...
        switch (instruction.op) {
            ...
            .ieq, .ine, .ugt, .uge, .ult, .ule, .sgt, .sge, .slt, .sle => {
                if (instruction.ty.scalar != .bool or instruction.ty.columns < 1 or instruction.ty.columns > 4 or instruction.ty.rows != 1) return error.InvalidType;
                const left = program.instructions[instruction.operands[0]].ty;
                const right = program.instructions[instruction.operands[1]].ty;
                if (!same(left, right) or left.columns != instruction.ty.columns or left.rows != instruction.ty.rows or left.columns < 1 or left.columns > 4 or left.rows != 1 or (left.scalar != .i32 and left.scalar != .u32)) return error.InvalidType;
                switch (instruction.op) {
            ...
            .ford_eq, .funord_eq, .ford_ne, .funord_ne, .ford_lt, .funord_lt, .ford_gt, .funord_gt, .ford_le, .funord_le, .ford_ge, .funord_ge => {
                ...
                const left = program.instructions[instruction.operands[0]].ty;
                const right = program.instructions[instruction.operands[1]].ty;
                if (!same(left, right) or left.columns != instruction.ty.columns or left.rows != instruction.ty.rows or left.scalar != .f32 or left.columns < 1 or left.columns > 4 or left.rows != 1) return error.InvalidType;
            },
            .logical_eq, .logical_ne, .logical_or, .logical_and => {
...
fn isValueOperand(op: ir.Op, i: usize) bool {
    return switch (op) {
        ...
        .copy_object => i == 0,
        .output => i == 1,
        .f_modf, .f_frexp, .f_modf_struct, .f_frexp_struct => i == 0,
        else => true,
    };
}
...
test "GLSL Ldexp scales f32 lanes with signed exponents and stays failure atomic" {
    var values = [_]u8{0} ** 16;
    var exponents = [_]u8{0} ** 16;
    const value_bits = [_]u32{ @bitCast(@as(f32, 1.5)), @bitCast(@as(f32, -2.0)), @bitCast(@as(f32, 0.25)), @bitCast(@as(f32, 8.0)) };
    const exponent_bits = [_]i32{ 2, -1, 3, 0 };
    for (0..4) |lane| {
        std.mem.writeInt(u32, values[lane * 4 ..][0..4], value_bits[lane], .little);
        std.mem.writeInt(u32, exponents[lane * 4 ..][0..4], @bitCast(exponent_bits[lane]), .little);
    }
    var interfaces = [_]ir.Interface{
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 0 },
        .{ .storage = .input, .ty = .{ .scalar = .i32, .columns = 4 }, .location = 1 },
    ...
    var instructions = [_]ir.Instruction{
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{0}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .i32, .columns = 4 }, .operands = &.{1}, .literal = &.{} },
        .{ .op = .f_ldexp, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
    defer executor.deinit();
    const bindings = [_]Binding{ .{ .interface = 0, .bytes = &values }, .{ .interface = 1, .bytes = &exponents } };
    try executor.execute(&bindings, &.{});
    const expected = [_]f32{ 6.0, -1.0, 2.0, 8.0 };
    for (expected, 0..) |value, lane| try std.testing.expectApproxEqAbs(value, @as(f32, @bitCast(executor.values[2].bits[lane])), 0.000001);
    const prior = executor.values[2];
    std.mem.writeInt(u32, exponents[12..16], @bitCast(@as(i32, 129)), .little);
...
test "GLSL ModfStruct returns fraction then integral scalar members" {
    const literal = f32bytes(-2.75);
    var interfaces = [_]ir.Interface{};
    var instructions = [_]ir.Instruction{
        .{ .op = .constant, .ty = .{ .scalar = .f32 }, .operands = &.{}, .literal = &literal },
        .{ .op = .f_modf_struct, .ty = .{ .scalar = .f32, .columns = 2 }, .operands = &.{0}, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
...
test "GLSL FrexpStruct returns significand then signed exponent scalar members" {
    const literal = f32bytes(-12.0);
    var interfaces = [_]ir.Interface{};
    var instructions = [_]ir.Instruction{
        .{ .op = .constant, .ty = .{ .scalar = .f32 }, .operands = &.{}, .literal = &literal },
        .{ .op = .f_frexp_struct, .ty = .{ .scalar = .f32, .columns = 2 }, .operands = &.{0}, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
    ...
    try executor.execute(&.{}, &.{});
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, -0.75))), executor.values[1].bits[0]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(i32, 4))), executor.values[1].bits[1]);
    for (0..4096) |_| try executor.execute(&.{}, &.{});
    const inf_literal = f32bytes(std.math.inf(f32));
    instructions[0].literal = &inf_literal;
    var invalid_source = try testProgram(&interfaces, &instructions);
...
test "GLSL NMin NMax and NClamp prefer non-NaN operands" {
    var x = [_]u8{0} ** 16;
    var y = [_]u8{0} ** 16;
    var minimum = [_]u8{0} ** 16;
    var maximum = [_]u8{0} ** 16;
    const x_values = [_]u32{ 0x7fc00000, @bitCast(@as(f32, 2)), 0x7fc00000, 0x80000000 };
    const y_values = [_]u32{ @bitCast(@as(f32, 1)), 0x7fc00000, 0x7fc00000, 0 };
    const min_values = [_]f32{ 0, 0, 1, -1 };
    const max_values = [_]f32{ 2, 2, 3, 1 };
    for (0..4) |lane| {
        std.mem.writeInt(u32, x[lane * 4 ..][0..4], x_values[lane], .little);
        std.mem.writeInt(u32, y[lane * 4 ..][0..4], y_values[lane], .little);
        std.mem.writeInt(u32, minimum[lane * 4 ..][0..4], @bitCast(min_values[lane]), .little);
        std.mem.writeInt(u32, maximum[lane * 4 ..][0..4], @bitCast(max_values[lane]), .little);
    }
    var interfaces = [_]ir.Interface{
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 0 },
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 1 },
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 2 },
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 3 },
    };
    var instructions = [_]ir.Instruction{
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{0}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{1}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{2}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{3}, .literal = &.{} },
        .{ .op = .f_n_min, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .f_n_max, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .f_n_clamp, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{ 0, 2, 3 }, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
    ...
    const bindings = [_]Binding{
        .{ .interface = 0, .bytes = &x },
        .{ .interface = 1, .bytes = &y },
        .{ .interface = 2, .bytes = &minimum },
        .{ .interface = 3, .bytes = &maximum },
    };
    try executor.execute(&bindings, &.{});
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 1))), executor.values[4].bits[0]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 2))), executor.values[4].bits[1]);
    try std.testing.expectEqual(@as(u32, 0x7fc00000), executor.values[4].bits[2]);
    try std.testing.expectEqual(@as(u32, 0x80000000), executor.values[4].bits[3]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 1))), executor.values[5].bits[0]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 2))), executor.values[5].bits[1]);
    try std.testing.expectEqual(@as(u32, 0x7fc00000), executor.values[5].bits[2]);
    try std.testing.expectEqual(@as(u32, 0x80000000), executor.values[5].bits[3]);
    try std.testing.expectEqual(@as(u32, 0), executor.values[6].bits[0]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 2))), executor.values[6].bits[1]);
    try std.testing.expectEqual(@as(u32, @bitCast(@as(f32, 1))), executor.values[6].bits[2]);
    try std.testing.expectEqual(@as(u32, 0x80000000), executor.values[6].bits[3]);
    const prior = executor.values[6];
    std.mem.writeInt(u32, maximum[0..4], @bitCast(@as(f32, -1)), .little);
    try std.testing.expectError(error.NumericDomain, executor.execute(&bindings, &.{}));
...
test "GLSL normalized pack and unpack operations preserve lane order" {
    ...
    var half_values2 = [_]u8{0} ** 8;
    var packed_bytes = [_]u8{0} ** 4;
    var half_packed_bytes = [_]u8{0} ** 4;
    ...
    std.mem.writeInt(u32, half_values2[0..4], 0x7fc12345, .little);
    std.mem.writeInt(u32, half_values2[4..8], 0x7f800000, .little);
    std.mem.writeInt(u32, &packed_bytes, 0x7f40c081, .little);
    std.mem.writeInt(u32, &half_packed_bytes, 0x3c00c000, .little);
    var interfaces = [_]ir.Interface{
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .location = 0 },
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 2 }, .location = 1 },
        .{ .storage = .input, .ty = .{ .scalar = .u32 }, .location = 2 },
        .{ .storage = .input, .ty = .{ .scalar = .u32 }, .location = 3 },
        .{ .storage = .input, .ty = .{ .scalar = .f32, .columns = 2 }, .location = 4 },
    };
    var instructions = [_]ir.Instruction{
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{0}, .literal = &.{} },
        ...
        .{ .op = .input, .ty = .{ .scalar = .u32 }, .operands = &.{3}, .literal = &.{} },
        .{ .op = .i_pack_half2x16, .ty = .{ .scalar = .u32 }, .operands = &.{1}, .literal = &.{} },
        .{ .op = .f_unpack_half2x16, .ty = .{ .scalar = .f32, .columns = 2 }, .operands = &.{11}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 2 }, .operands = &.{4}, .literal = &.{} },
        .{ .op = .i_pack_half2x16, .ty = .{ .scalar = .u32 }, .operands = &.{14}, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
    defer executor.deinit();
    const bindings = [_]Binding{ .{ .interface = 0, .bytes = &values4 }, .{ .interface = 1, .bytes = &values2 }, .{ .interface = 2, .bytes = &packed_bytes }, .{ .interface = 3, .bytes = &half_packed_bytes }, .{ .interface = 4, .bytes = &half_values2 } };
    try executor.execute(&bindings, &.{});
    ...
    try std.testing.expectEqual(@as(u32, 0xb800bc00), executor.values[12].bits[0]);
    try std.testing.expectApproxEqAbs(@as(f32, -2), @as(f32, @bitCast(executor.values[13].bits[0])), 0.000001);
    try std.testing.expectApproxEqAbs(@as(f32, 1), @as(f32, @bitCast(executor.values[13].bits[1])), 0.000001);
    ...
    try executor.execute(&bindings, &.{});
    try std.testing.expectEqual(@as(u32, 0x7c007e00), executor.values[15].bits[0]);
    for (0..4096) |_| try executor.execute(&bindings, &.{});
}
...
test "integer and floating comparisons preserve vector lanes and signedness" {
    var signed_a = [_]u8{0} ** 16;
    var signed_b = [_]u8{0} ** 16;
    var float_a = [_]u8{0} ** 16;
    var float_b = [_]u8{0} ** 16;
    const ia = [_]i32{ -2, 0, 3, 4 };
    const ib = [_]i32{ -1, 0, 2, 5 };
    const fa = [_]u32{ 0x7fc12345, 0x80000000, @bitCast(@as(f32, 1)), 0x7f800000 };
    const fb = [_]u32{ 0, 0, @bitCast(@as(f32, 2)), 0x7f800000 };
    for (0..4) |lane| {
        std.mem.writeInt(u32, signed_a[lane * 4 ..][0..4], @bitCast(ia[lane]), .little);
        std.mem.writeInt(u32, signed_b[lane * 4 ..][0..4], @bitCast(ib[lane]), .little);
        std.mem.writeInt(u32, float_a[lane * 4 ..][0..4], fa[lane], .little);
        std.mem.writeInt(u32, float_b[lane * 4 ..][0..4], fb[lane], .little);
    }
    var interfaces = [_]ir.Interface{
        .{ .storage = .input, .ty = .{ .scalar = .i32, .columns = 4 }, .location = 0 },
        .{ .storage = .input, .ty = .{ .scalar = .i32, .columns = 4 }, .location = 1 },
    ...
    var instructions = [_]ir.Instruction{
        ...
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{2}, .literal = &.{} },
        .{ .op = .input, .ty = .{ .scalar = .f32, .columns = 4 }, .operands = &.{3}, .literal = &.{} },
        .{ .op = .ieq, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .ine, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .sgt, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .slt, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 0, 1 }, .literal = &.{} },
        .{ .op = .ford_eq, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 2, 3 }, .literal = &.{} },
        .{ .op = .funord_eq, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 2, 3 }, .literal = &.{} },
        .{ .op = .ford_lt, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 2, 3 }, .literal = &.{} },
        .{ .op = .funord_lt, .ty = .{ .scalar = .bool, .columns = 4 }, .operands = &.{ 2, 3 }, .literal = &.{} },
    };
    var source = try testProgram(&interfaces, &instructions);
    defer std.testing.allocator.free(source.bytes);
    var executor = try Executor.init(std.testing.allocator, &source);
    defer executor.deinit();
    const bindings = [_]Binding{ .{ .interface = 0, .bytes = &signed_a }, .{ .interface = 1, .bytes = &signed_b }, .{ .interface = 2, .bytes = &float_a }, .{ .interface = 3, .bytes = &float_b } };
    try executor.execute(&bindings, &.{});
    try std.testing.expectEqualSlices(u32, &.{ 0, 1, 0, 0 }, executor.values[4].bits[0..4]);
    try std.testing.expectEqualSlices(u32, &.{ 1, 0, 1, 1 }, executor.values[5].bits[0..4]);
    try std.testing.expectEqualSlices(u32, &.{ 0, 0, 1, 0 }, executor.values[6].bits[0..4]);
    try std.testing.expectEqualSlices(u32, &.{ 1, 0, 0, 1 }, executor.values[7].bits[0..4]);
    try std.testing.expectEqualSlices(u32, &.{ 0, 1, 0, 1 }, executor.values[8].bits[0..4]);
    try std.testing.expectEqualSlices(u32, &.{ 1, 1, 0, 1 }, executor.values[9].bits[0..4]);
...
fn runPropertyCase(op: ir.Op, result_ty: ir.Type, source_ty_override: ?ir.Type, convert_from: ?ir.Scalar) !void {
    ...
    switch (op) {
        ...
        .f_ldexp => {
            const source_ty = source_ty_override orelse return error.InvalidType;
            const value = try propertyConstant(arena, &instructions, source_ty);
        ...
        .f_modf_struct => {
            const source = try propertyConstant(arena, &instructions, .{ .scalar = .f32 });
            result_id = try propertyInstruction(arena, &instructions, op, result_ty, &.{source}, &.{});
        },
        .f_frexp_struct => {
            const source = try propertyConstant(arena, &instructions, .{ .scalar = .f32 });
        ...
        .iadd, .isub, .imul, .bit_or, .bit_xor, .bit_and, .udiv, .sdiv, .umod, .srem, .smod, .shl_logical, .shr_logical, .shr_arithmetic, .f_atan2, .f_pow, .fadd, .fsub, .fmul, .fdiv, .frem, .fmod, .f_min, .f_max, .f_n_min, .f_n_max, .f_step => {
            const a = try propertyConstant(arena, &instructions, result_ty);
            const b = try propertyConstant(arena, &instructions, result_ty);
            result_id = try propertyInstruction(arena, &instructions, op, result_ty, &.{ a, b }, &.{});
        },
...
test "generated bounded operation by type-family property matrix is complete" {
    ...
    for (property_types) |ty| {
        ...
        if (is_float and is_non_matrix) inline for ([_]ir.Op{ .f_min, .f_max, .f_n_min, .f_n_max }) |op| {
            try runPropertyCase(op, ty, null, null);
            totals[@intFromEnum(op)] += 1;
        };
        if (is_float and is_non_matrix) {
            ...
            try runPropertyCase(.f_n_clamp, ty, null, null);
            totals[@intFromEnum(ir.Op.f_n_clamp)] += 1;
    ...
    try runPropertyCase(.i_pack_half2x16, .{ .scalar = .u32 }, null, null);
    totals[@intFromEnum(ir.Op.i_pack_half2x16)] += 1;
    try runPropertyCase(.f_unpack_half2x16, .{ .scalar = .f32, .columns = 2 }, null, null);
    totals[@intFromEnum(ir.Op.f_unpack_half2x16)] += 1;
    try runPropertyCase(.f_modf_struct, .{ .scalar = .f32, .columns = 2 }, null, null);
    totals[@intFromEnum(ir.Op.f_modf_struct)] += 1;
    try runPropertyCase(.f_frexp_struct, .{ .scalar = .f32, .columns = 2 }, null, null);
    totals[@intFromEnum(ir.Op.f_frexp_struct)] += 1;
