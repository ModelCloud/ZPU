...
fn createGraphicsPipelines(device: ?Device, cache: usize, count: u32, infos: ?*const anyopaque, alloc: ?*const Alloc, outputs: ?[*]usize) callconv(.c) Result {
    ...
    for (0..count) |index| built[index] = buildGraphicsPipelineLocked(d, &create_infos[index]) catch |err| return creationFailure(err);
    for (0..count) |index| {
...
test "vkcube presentation path records submits and presents two swapchain images" {
    ...
    _ = try buildPipelineLayoutLocked(device, &push_layout_info);
    ...
    _ = try buildRenderPass(&extended_render_info, false);
