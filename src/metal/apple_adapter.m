...
@public
    ...
    zpu_metal_compute_kernel _kernel;
    /* Metal keeps texture bindings per index. The bounded CPU profiles use
     * texture 0 for gradient/array/3D/trace kernels and texture 1 for the
...
static BOOL zpu_integer_render_format_supported(MTLPixelFormat format) {
    return format == MTLPixelFormatR8Uint || format == MTLPixelFormatR8Sint ||
        format == MTLPixelFormatR16Uint || format == MTLPixelFormatR16Sint ||
        format == MTLPixelFormatRG8Uint || format == MTLPixelFormatRG8Sint ||
        format == MTLPixelFormatRG16Uint || format == MTLPixelFormatRG16Sint ||
        format == MTLPixelFormatRGBA8Uint || format == MTLPixelFormatRGBA8Sint ||
...
static BOOL zpu_color_store_action_supported(MTLStoreAction action, BOOL multisample, BOOL hasResolve) {
    if (zpu_store_action_requests_resolve(action)) return multisample && hasResolve;
    return zpu_store_action_supported(action);
}

static BOOL zpu_configure_multisample_targets(ZPUCommandBuffer *owner,
    ...
    if (sampleCount == 1) return !hasResolve && !zpu_store_action_requests_resolve(attachment.storeAction);
    if (texture->_textureType != MTLTextureType2DMultisample ||
        (sampleCount != 2 && sampleCount != 4) || attachment.level != 0 || attachment.slice != 0 ||
        !zpu_color_store_action_supported(attachment.storeAction, YES, hasResolve)) return NO;
    if (zpu_store_action_requests_resolve(attachment.storeAction) != hasResolve ||
    ...
    for (NSUInteger index = 0; index < sampleCount; ++index) {
        ...
        if (samples[index] == NULL) return NO;
    }
    ...
    if (zpu_metal_render_encoder_set_multisample_targets(encoder, samples, sampleCount, resolveTexture) != ZPU_METAL_OK) return NO;
...
static BOOL zpu_configure_multisample_depth_targets(zpu_metal_render_encoder *encoder,
    ...
    return zpu_metal_render_encoder_set_multisample_depth_targets(encoder, samples, sampleCount) == ZPU_METAL_OK;
}
...
static BOOL zpu_configure_multisample_stencil_targets(zpu_metal_render_encoder *encoder,
                                                       ...
                                                       NSUInteger sampleCount,
                                                       id attachment) {
    if (encoder == NULL || texture == nil || attachment == nil || texture.sampleCount != sampleCount ||
        texture->_textureType != MTLTextureType2DMultisample ||
        (sampleCount != 2 && sampleCount != 4)) return NO;
    zpu_metal_texture *samples[4] = {NULL, NULL, NULL, NULL};
    for (NSUInteger index = 0; index < sampleCount; ++index) {
    ...
    return zpu_metal_render_encoder_set_multisample_stencil_targets(
        encoder, samples, sampleCount, zpu_load_action([attachment loadAction]),
        zpu_store_action([attachment storeAction]), (uint8_t)[attachment clearStencil]) == ZPU_METAL_OK;
}
...
static BOOL zpu_render_pipeline_format_supported_for_fragment(MTLPixelFormat format, NSString *fragmentName) {
    ...
    if (zpu_color_texture_format_supported(format)) {
        return ![fragmentName hasSuffix:@"_uint_fragment"] && ![fragmentName hasSuffix:@"_sint_fragment"];
    }
    if ([fragmentName isEqualToString:@"zpu_cpu_r8_uint_fragment"]) return format == MTLPixelFormatR8Uint;
    if ([fragmentName isEqualToString:@"zpu_cpu_r8_sint_fragment"]) return format == MTLPixelFormatR8Sint;
    if ([fragmentName isEqualToString:@"zpu_cpu_r16_uint_fragment"]) return format == MTLPixelFormatR16Uint;
    if ([fragmentName isEqualToString:@"zpu_cpu_r16_sint_fragment"]) return format == MTLPixelFormatR16Sint;
    if ([fragmentName isEqualToString:@"zpu_cpu_rg8_uint_fragment"]) return format == MTLPixelFormatRG8Uint;
...
static BOOL zpu_texture_descriptor_size(MTLTextureDescriptor *descriptor, NSUInteger *size) {
    ...
    if (multisample) {
        if (total != 0 && descriptor.sampleCount > SIZE_MAX / total) return NO;
        total *= descriptor.sampleCount;
    }
    *size = total;
    return YES;
}
...
static BOOL zpu_metal4_render_pass_descriptor(ZPUDevice *owner,
    ...
    const BOOL multisample = color.sampleCount > 1;
...
static NSArray *zpu_make_additional_sample_planes(ZPUDevice *owner, ZPUHeap *heap,
                                                  ...
                                                  BOOL placementSparse, BOOL *success) {
    NSMutableArray *planes = [NSMutableArray arrayWithCapacity:sampleCount > 0 ? sampleCount - 1 : 0];
...
- (NSUInteger)sampleCount { return _backing != nil ? [_backing sampleCount] : _sampleCount; }
...
static BOOL zpu_cpu_acceleration_instance_triangle_count(
    MTLInstanceAccelerationStructureDescriptor *descriptor, NSUInteger *triangleCount);
...
static NSUInteger zpu_acceleration_structure_size_for_descriptor(
    ...
    if ([descriptor isKindOfClass:[MTLInstanceAccelerationStructureDescriptor class]]) {
        ...
        if (zpu_cpu_acceleration_instance_triangle_count(
                (MTLInstanceAccelerationStructureDescriptor *)descriptor, &instanceTriangleCount)) {
...
- (id<MTLTexture>)zpuNewTextureWithDescriptor:(MTLTextureDescriptor *)descriptor firstOffset:(NSUInteger)offset explicitOffset:(BOOL)explicitOffset {
    ...
    if (@available(macOS 26.0, iOS 26.0, *)) {
        const NSInteger sparsePageSize = (NSInteger)descriptor.placementSparsePageSize;
        if (sparsePageSize != 0) {
            if (descriptor.sampleCount != 1) return nil;
...
static MTLFunctionReflection *zpu_function_reflection(NSString *name) {
    ...
    if ([name isEqualToString:@"zpu_test_stage_in_vertex"] ||
        ...
        [name isEqualToString:@"zpu_cpu_r8_sint_fragment"] ||
        [name isEqualToString:@"zpu_cpu_r16_uint_fragment"] ||
        [name isEqualToString:@"zpu_cpu_r16_sint_fragment"] ||
        [name isEqualToString:@"zpu_cpu_rg8_uint_fragment"] ||
...
static BOOL zpu_cpu_function_name_supported(NSString *name) {
    return [@[
        ...
        @"zpu_cpu_r16_sint_fragment",
        @"zpu_cpu_rg8_uint_fragment",
    ...
    for (NSUInteger index = 0; index < ZPU_METAL_MAX_COLOR_ATTACHMENTS; ++index) {
        if (!zpu_render_pipeline_format_supported_for_fragment(
            ...
            (descriptor.colorAttachments[index].blendingEnabled &&
             zpu_integer_render_format_supported(descriptor.colorAttachments[index].pixelFormat))) {
            zpu_set_error(error, @"ZPU Metal supports the registered CPU integer fragment profiles without blending, plus the normalized and float CPU color targets");
            return nil;
        }
    }
    if (error != NULL) *error = nil;
...
- (instancetype)initWithOwner:(ZPUDevice *)owner name:(NSString *)name {
    return [self initWithOwner:owner name:name functionType:0 options:MTLFunctionOptionNone];
}
- (instancetype)initWithOwner:(ZPUDevice *)owner name:(NSString *)name
                  functionType:(MTLFunctionType)functionType {
    return [self initWithOwner:owner name:name functionType:functionType options:MTLFunctionOptionNone];
}
- (instancetype)initWithOwner:(ZPUDevice *)owner name:(NSString *)name
                  functionType:(MTLFunctionType)functionType
                       options:(MTLFunctionOptions)options {
...
static id<MTLRenderPipelineState> zpu_mtl4_render_pipeline_for_descriptor(
    ...
    if (descriptor.rasterSampleCount != 0 && descriptor.rasterSampleCount != 1 &&
        ...
        zpu_set_error(error, @"ZPU CPU Metal 4 supports only 1x, 2x, and 4x CPU render targets");
        return nil;
    }
...
- (id<MTLRenderCommandEncoder>)renderCommandEncoderWithDescriptor:(MTLRenderPassDescriptor *)descriptor {
    ...
    if (multisample) {
        for (NSUInteger index = 1; index < ZPU_METAL_MAX_COLOR_ATTACHMENTS; ++index) {
            if (descriptor.colorAttachments[index].texture != nil) return nil;
        }
    }
    ...
    if (descriptor.depthAttachment.texture != nil) {
        ...
        if (!multisample && zpu_metal_render_encoder_set_depth_texture(encoder, depthTexture) != ZPU_METAL_OK) {
            zpu_metal_render_encoder_destroy(encoder);
            return nil;
        }
    }
    if (descriptor.stencilAttachment.texture != nil) {
        ...
        if (!multisample && zpu_metal_render_encoder_set_stencil_texture(encoder, stencilTexture,
...
- (id<MTLParallelRenderCommandEncoder>)parallelRenderCommandEncoderWithDescriptor:(MTLRenderPassDescriptor *)descriptor {
    ...
    if (![texture isKindOfClass:[ZPUTexture class]] || !zpu_render_texture_type_supported(texture->_textureType)) return nil;
    /* The bounded CPU profile resolves MSAA at the ordered render-pass
     * boundary. Parallel child encoders do not expose a reliable final-child
     * signal, so keep their multisample behavior fail-closed for now. */
    if (texture.sampleCount > 1) return nil;
    zpu_metal_texture *colorTexture = [texture zpuTextureAtLevel:colorAttachment.texture != nil ? colorAttachment.level : 0
                                                            slice:colorAttachment.texture != nil ? colorAttachment.slice : 0];
    if (colorTexture == NULL) return nil;
    zpu_metal_texture *depthTexture = NULL;
...
- (id<MTL4RenderCommandEncoder>)renderCommandEncoderWithDescriptor:(MTL4RenderPassDescriptor *)descriptor options:(MTL4RenderEncoderOptions)options {
    ...
    if (depth != nil) {
        ...
        if (color.sampleCount == 1 && zpu_metal_render_encoder_set_depth_texture(encoder, depthTexture) != ZPU_METAL_OK) {
            zpu_metal_render_encoder_destroy(encoder);
            [self markError];
            return nil;
        }
    ...
    if (stencil != nil) {
        ...
        if (color.sampleCount == 1 && zpu_metal_render_encoder_set_stencil_texture(encoder, stencilTexture,
                zpu_load_action(descriptor.stencilAttachment.loadAction),
                zpu_store_action(descriptor.stencilAttachment.storeAction),
                (uint8_t)descriptor.stencilAttachment.clearStencil) != ZPU_METAL_OK) {
            zpu_metal_render_encoder_destroy(encoder);
...
- (void)dispatchThreadsWithIndirectBuffer:(MTLGPUAddress)indirectBuffer {
    ...
    ZPUTexture *boundTexture = zpu_compute_bound_texture(_legacy);
    const NSUInteger boundTextureIndex = zpu_compute_texture_index_for_kernel(_legacy->_kernel);
    const BOOL arrayKernel = _legacy->_kernel == ZPU_METAL_COMPUTE_FILL_GRADIENT_RGBA8_ARRAY;
    if (arrayKernel) {
        if (boundTexture == nil || boundTexture->_textureType != MTLTextureType2DArray) {
            [_owner markError];
            return;
        }
        for (NSUInteger slice = 0; slice < boundTexture.arrayLength; ++slice) {
            zpu_metal_texture *sliceTexture = [boundTexture zpuTextureAtLevel:0 slice:slice];
            if (sliceTexture == NULL ||
                zpu_metal_compute_encoder_set_texture(_legacy->_zpuEncoder, sliceTexture,
                    (uint32_t)boundTextureIndex) != ZPU_METAL_OK ||
        ...
        if (boundTexture == nil || boundTexture->_textureType != MTLTextureType3D) {
            [_owner markError];
            return;
        }
        for (NSUInteger slice = 0; slice < boundTexture.depth; ++slice) {
            zpu_metal_texture *sliceTexture = [boundTexture zpuTextureAtLevel:0 slice:slice];
            if (sliceTexture == NULL ||
                zpu_metal_compute_encoder_set_texture(_legacy->_zpuEncoder, sliceTexture,
                    (uint32_t)boundTextureIndex) != ZPU_METAL_OK ||
    ...
    } else if (boundTexture != nil && (boundTexture->_textureType == MTLTextureType2DArray ||
                                       boundTexture->_textureType == MTLTextureType3D)) {
        [_owner markError];
        return;
...
- (void)setTexture:(id<MTLTexture>)texture atIndex:(NSUInteger)index {
    ...
    _boundTextures[index] = zpuTexture;
...
- (void)dispatchThreads:(MTLSize)threadsPerGrid threadsPerThreadgroup:(MTLSize)threadsPerThreadgroup API_AVAILABLE(macos(10.13), ios(11.0), tvos(14.5)) {
    ...
    const NSUInteger boundTextureIndex = zpu_compute_texture_index_for_kernel(_kernel);
    if (arrayKernel) {
        if (boundTexture == nil || boundTexture->_textureType != MTLTextureType2DArray) {
            [_owner markError];
            return;
        }
        for (NSUInteger slice = 0; slice < threadsPerGrid.depth && slice < boundTexture.arrayLength; ++slice) {
            zpu_metal_texture *sliceTexture = [boundTexture zpuTextureAtLevel:0 slice:slice];
            if (sliceTexture == NULL ||
                zpu_metal_compute_encoder_set_texture(_zpuEncoder, sliceTexture, (uint32_t)boundTextureIndex) != ZPU_METAL_OK ||
    ...
    if (volumeKernel) {
        ...
        for (NSUInteger slice = 0; slice < threadsPerGrid.depth && slice < boundTexture.depth; ++slice) {
            zpu_metal_texture *sliceTexture = [boundTexture zpuTextureAtLevel:0 slice:slice];
            if (sliceTexture == NULL ||
                zpu_metal_compute_encoder_set_texture(_zpuEncoder, sliceTexture, (uint32_t)boundTextureIndex) != ZPU_METAL_OK ||
...
- (void)dispatchThreadgroups:(MTLSize)threadgroupsPerGrid threadsPerThreadgroup:(MTLSize)threadsPerThreadgroup {
    ...
    if (_kernel == ZPU_METAL_COMPUTE_FILL_GRADIENT_RGBA8_3D &&
        boundTexture != nil && boundTexture->_textureType == MTLTextureType3D) {
        const uint64_t gridWidth = (uint64_t)threadgroupsPerGrid.width * threadsPerThreadgroup.width;
        const uint64_t gridHeight = (uint64_t)threadgroupsPerGrid.height * threadsPerThreadgroup.height;
        if (gridWidth > UINT32_MAX || gridHeight > UINT32_MAX) {
            [_owner markError];
...
static BOOL zpu_cpu_acceleration_payload_info(ZPUAccelerationStructure *structure,
                                               NSUInteger *triangleCount,
                                               NSUInteger *triangleOffset) {
    if (structure == nil || !zpu_acceleration_storage_range_valid(structure, structure->_size) ||
        structure->_size < sizeof(zpu_metal_cpu_acceleration_structure_header) ||
        triangleCount == NULL || triangleOffset == NULL) return NO;
    zpu_metal_cpu_acceleration_structure_header header;
    memcpy(&header, structure->_storage.contents, sizeof(header));
    if (header.magic != ZPU_METAL_CPU_ACCELERATION_STRUCTURE_MAGIC ||
        header.version != ZPU_METAL_CPU_ACCELERATION_STRUCTURE_VERSION ||
        header.triangle_offset < ZPU_METAL_CPU_ACCELERATION_STRUCTURE_HEADER_BYTES) return NO;
    const NSUInteger offset = header.triangle_offset;
    const NSUInteger count = header.triangle_count;
    if (offset > structure->_size ||
        count > (structure->_size - offset) / sizeof(zpu_metal_cpu_acceleration_triangle)) return NO;
    *triangleCount = count;
    *triangleOffset = offset;
    return YES;
}

static BOOL zpu_cpu_acceleration_instance_descriptor_size(
    MTLInstanceAccelerationStructureDescriptor *descriptor, NSUInteger *descriptorSize) {
    if (descriptor == nil || descriptorSize == NULL) return NO;
    NSUInteger size = sizeof(MTLAccelerationStructureInstanceDescriptor);
    if (@available(macOS 12.0, iOS 15.0, *)) {
        switch (descriptor.instanceDescriptorType) {
            case MTLAccelerationStructureInstanceDescriptorTypeDefault:
                break;
            case MTLAccelerationStructureInstanceDescriptorTypeUserID:
                size = sizeof(MTLAccelerationStructureUserIDInstanceDescriptor);
                break;
            default:
                return NO;
        }
...
static BOOL zpu_cpu_acceleration_instance_triangle_count(
    MTLInstanceAccelerationStructureDescriptor *descriptor, NSUInteger *triangleCount) {
    NSUInteger descriptorSize = 0;
    if (!zpu_cpu_acceleration_instance_descriptor_size(descriptor, &descriptorSize) ||
        triangleCount == NULL) return NO;
    NSArray *children = descriptor.instancedAccelerationStructures;
    if (descriptor.instanceCount > 0 && descriptor.instanceCount > children.count) return NO;
    NSUInteger total = 0;
    for (NSUInteger index = 0; index < descriptor.instanceCount; ++index) {
        id value = children[index];
        if (![value isKindOfClass:[ZPUAccelerationStructure class]]) return NO;
        ZPUAccelerationStructure *child = (ZPUAccelerationStructure *)value;
        ...
        if (child->_built && zpu_cpu_acceleration_payload_info(child, &childCount, &childOffset)) {
            ...
            /* A descriptor can be sized before its child is built. Use the
             * child's CPU allocation as a conservative upper bound so the
             * later flattening step cannot run out of target storage. */
...
static BOOL zpu_cpu_acceleration_transform_position(const MTLPackedFloat4x3 *matrix,
                                                     const float input[3], float output[3]) {
    if (matrix == NULL || input == NULL || output == NULL) return NO;
    output[0] = matrix->columns[0].x * input[0] + matrix->columns[1].x * input[1] +
        matrix->columns[2].x * input[2] + matrix->columns[3].x;
    output[1] = matrix->columns[0].y * input[0] + matrix->columns[1].y * input[1] +
        matrix->columns[2].y * input[2] + matrix->columns[3].y;
    output[2] = matrix->columns[0].z * input[0] + matrix->columns[1].z * input[1] +
        matrix->columns[2].z * input[2] + matrix->columns[3].z;
...
static BOOL zpu_cpu_acceleration_write_instance_payload(
    ...
    NSArray *children = descriptor.instancedAccelerationStructures;
    if (descriptor.instanceCount > 0 && descriptor.instanceCount > children.count) return NO;
    ZPUBuffer *instanceBuffer = (ZPUBuffer *)descriptor.instanceDescriptorBuffer;
    const NSUInteger instanceOffset = descriptor.instanceDescriptorBufferOffset;
    const NSUInteger instanceStride = descriptor.instanceDescriptorStride == 0 ? descriptorSize :
        descriptor.instanceDescriptorStride;
    if (descriptor.instanceCount > 0 &&
        (!zpu_buffer_belongs_to_device(target->_owner, instanceBuffer) || instanceBuffer.contents == NULL ||
         instanceOffset % 64 != 0 || instanceStride < descriptorSize || instanceStride % 4 != 0 ||
         instanceOffset > instanceBuffer.length ||
         descriptor.instanceCount > (instanceBuffer.length - instanceOffset) / instanceStride)) return NO;

    NSUInteger triangleCount = 0;
    for (NSUInteger index = 0; index < descriptor.instanceCount; ++index) {
        ZPUAccelerationStructure *child = (ZPUAccelerationStructure *)children[index];
        NSUInteger childCount = 0;
        NSUInteger childOffset = 0;
        if (![child isKindOfClass:[ZPUAccelerationStructure class]] || child->_owner != target->_owner ||
            !child->_built || !zpu_cpu_acceleration_payload_info(child, &childCount, &childOffset) ||
    ...
    for (NSUInteger index = 0; index < descriptor.instanceCount; ++index) {
        const uint8_t *instanceBytes = (const uint8_t *)instanceBuffer.contents + instanceOffset +
            index * instanceStride;
        MTLAccelerationStructureInstanceDescriptor instance;
        memcpy(&instance, instanceBytes, sizeof(instance));
        if (instance.options != MTLAccelerationStructureInstanceOptionNone || instance.mask != UINT32_MAX ||
            ...
            instance.accelerationStructureIndex >= children.count) return NO;
        ZPUAccelerationStructure *child = (ZPUAccelerationStructure *)children[instance.accelerationStructureIndex];
        NSUInteger childCount = 0;
        NSUInteger childOffset = 0;
        if (![child isKindOfClass:[ZPUAccelerationStructure class]] || child->_owner != target->_owner ||
            !child->_built || !zpu_cpu_acceleration_payload_info(child, &childCount, &childOffset)) return NO;
...
API_AVAILABLE(macos(26.0), ios(26.0))
static BOOL zpu_cpu_acceleration_metal4_instance_buffer(
    MTL4InstanceAccelerationStructureDescriptor *descriptor, ZPUDevice *owner,
    ZPUBuffer **buffer, NSUInteger *offset, NSUInteger *rangeLength, NSUInteger *descriptorStride) {
    if (descriptor == nil || buffer == NULL || offset == NULL || rangeLength == NULL || descriptorStride == NULL ||
    ...
    if (stride < elementSize || stride % 4 != 0 ||
        !zpu_metal4_buffer_range(descriptor.instanceDescriptorBuffer, owner, buffer, offset)) return NO;
    const NSUInteger length = descriptor.instanceDescriptorBuffer.length == UINT64_MAX ?
        (*buffer).length - *offset : (NSUInteger)descriptor.instanceDescriptorBuffer.length;
    if (descriptor.instanceCount > length / stride) return NO;
    *rangeLength = length;
    *descriptorStride = stride;
    return (*buffer).contents != NULL;
}

API_AVAILABLE(macos(26.0), ios(26.0))
static ZPUAccelerationStructure *zpu_cpu_acceleration_metal4_instance_child(
    const MTLIndirectAccelerationStructureInstanceDescriptor *instance, ZPUDevice *owner) {
    if (instance == NULL) return nil;
    id value = zpu_resource_for_id(instance->accelerationStructureID._impl);
    return [value isKindOfClass:[ZPUAccelerationStructure class]] &&
            ((ZPUAccelerationStructure *)value)->_owner == owner ? (ZPUAccelerationStructure *)value : nil;
}

API_AVAILABLE(macos(26.0), ios(26.0))
static BOOL zpu_cpu_acceleration_metal4_instance_triangle_count(
    MTL4InstanceAccelerationStructureDescriptor *descriptor, NSUInteger *triangleCount) {
    if (descriptor == nil || triangleCount == NULL || descriptor.instanceDescriptorBuffer.bufferAddress == 0) return NO;
    ...
    if (triangleCount == NULL || !zpu_cpu_acceleration_metal4_instance_buffer(
            descriptor, owner, &instanceBuffer, &instanceOffset,
            &instanceRangeLength, &instanceStride)) return NO;
    NSUInteger total = 0;
    for (NSUInteger index = 0; index < descriptor.instanceCount; ++index) {
...
API_AVAILABLE(macos(26.0), ios(26.0))
static BOOL zpu_cpu_acceleration_write_metal4_instance_payload(
    ...
    NSUInteger instanceRangeLength = 0;
    NSUInteger instanceStride = 0;
    if (!zpu_cpu_acceleration_metal4_instance_buffer(
            descriptor, target->_owner, &instanceBuffer, &instanceOffset,
            &instanceRangeLength, &instanceStride)) return NO;
    NSUInteger triangleCount = 0;
    for (NSUInteger index = 0; index < descriptor.instanceCount; ++index) {
        MTLIndirectAccelerationStructureInstanceDescriptor instance;
...
- (void)setRenderPipelineState:(id<MTLRenderPipelineState>)pipelineState {
    ...
    if (zpu_metal_render_encoder_set_pipeline_color_formats(
        _zpuEncoder, colorFormats, state->_colorAttachmentCount,
            (uint16_t)state->_depthPixelFormat, (uint16_t)state->_stencilPixelFormat) != ZPU_METAL_OK ||
        zpu_metal_render_encoder_set_raster_sample_count(_zpuEncoder, (uint8_t)state->_rasterSampleCount) != ZPU_METAL_OK) [_owner markError];
