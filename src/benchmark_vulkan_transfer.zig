...
const Report = struct {
    ...
    renderer_scope: []const u8 = "Vulkan buffer/image transfer rows; validated non-overlap bulk-copy path",
...
fn measure(source: []u8, destination: []u8, optimized: bool, smoke: bool, sink: *u64) Timing {
    ...
    for (0..warmups) |frame| {
        ...
        if (optimized) copyBulk(destination, source) else copyForwards(destination, source);
...
pub fn main(init: std.process.Init) !void {
    ...
    if (json) {
        ...
        std.debug.print("Vulkan transfer p50: copyForwards={}ns bulk={}ns ({d:.2}x) checksum={x:0>16}\n", .{ forwards.p50_ns, bulk.p50_ns, report.p50_speedup, checksum(destination) ^ sink });
    }
}

test "Vulkan transfer benchmark preserves pitched rows and layer boundaries" {
    var source: [2 * 8 * 4]u8 = undefined;
    var destination: [2 * 6 * 4]u8 = undefined;
    for (&source, 0..) |*byte, index| byte.* = @truncate(index * 13 + 7);
    @memset(&destination, 0);
    driver.benchmarkVulkanBufferImageCopy(&destination, &source, 6, 2, 8);
    for (0..2) |row| {
